...
inline FirstLayer processed_l1(const Board &board){
	return FirstLayer{
		...
		vector_clamp((wtm ? vector_add : vector_sub)(board.ue.l1.vert_asym, w_l1_tempo_va)),
...
inline SecondLayer l1_l2_transition(const FirstLayer l1){
	...
	l2_va = vector_add(l2_va, matmul(w_l2_fsxva_va, vector_mul(l1.full_symm, l1.vert_asym)));
	l2_va = vector_add(l2_va, matmul(w_l2_haxra_va, vector_mul(l1.horz_asym, l1.rotl_asym)));
...
inline int eval_from_l2(const SecondLayer l2){
	return vector_dot(l2.vert_asym, w_final_va) + vector_dot(vector_mul(l2.full_symm, l2.vert_asym), w_final_fsxva);
}
