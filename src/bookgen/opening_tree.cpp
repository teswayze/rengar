...
OpeningTree init_opening_tree(const std::string fen){
    ...
    leaf_node_map.insert(std::pair{get_key(board, true), root});

    Board board_copy = board.copy();
    make_move<true>(board_copy, first_move);
    first_oob_map.insert(std::pair{get_key(board_copy, false), root});
...
void OpeningTree::convert_leaf_to_interior(const int search_depth, const Board &board, const bool wtm){
    ...
    while (!move_queue.empty()) {
        ...
        if (interior_node_map.count(child_key)) {
            ...
            leaf_node_map.insert(std::pair{child_key, new_leaf});
    ...
    interior_node_map.insert(std::pair{leaf_key, interior_node});
...
bool OpeningTree::deepen_recursive(const int search_depth, Board &board, const bool wtm){
    ...
    node.book_exit = true;
    first_oob_map.insert(std::pair{first_oob_key, node});
