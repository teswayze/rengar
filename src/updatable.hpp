...
inline void update_first_layer(FirstLayer &layer, const Square square){
    ...
    layer.full_symm = (add ? vector_add : vector_sub)(layer.full_symm, to_update.full_symm);
    layer.vert_asym = ((add ^ white) ? vector_add : vector_sub)(layer.vert_asym, to_update.vert_asym);
    layer.horz_asym = ((add ^ flip_h) ? vector_add : vector_sub)(layer.horz_asym, to_update.horz_asym);
    layer.rotl_asym = ((add ^ white ^ flip_h) ? vector_add : vector_sub)(layer.rotl_asym, to_update.rotl_asym);
}
