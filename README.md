...
Rengar's network is trained by `pytorch`. The network used in v2.0 was trained using over 1 million games generated by selfplay from v1.3. The games started from chess324 positions and searched to depth 6 on each move.

The Engine's inference uses `Eigen`. All intermediate values are stored as 32-bit floats.
